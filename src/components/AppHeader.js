...
const Header = () => {
  ...
  const { network, token, user, userAuth, showDepositModal, setShowDepositModal, setIsLoginWindowOpen, isConnectedEVM, isConnectedSolana } = useContext(BaseContext)
  ...
  const isActive = (paths = []) => {
    return paths.some(x => x === router?.pathname)
  }
  ...
  useEffect(() => {
    ...
    const _menu1 = [
      { label: "Home", href: `/casinos/${network?.name}/${token?.address}`, icon: "home2", size: "25px", active: isActive(["/casinos/[network]/[token]"]) },
      { label: "Poker", href: `/casinos/${network?.name}/${token?.address}/poker`, icon: "cards", size: "25px", active: isActive(["/casinos/[network]/[token]/poker", "/casinos/[network]/[token]/poker/[room]"]) },
      { label: "Coinflip", href: `/casinos/${network?.name}/${token?.address}/coinflip`, icon: "coin", size: "25px", active: isActive(["/casinos/[network]/[token]/coinflip", "/casinos/[network]/[token]/coinflip/[roomid]"]) },
      { label: "Rock Paper Scissors", href: `/casinos/${network?.name}/${token?.address}/rockpaperscissors`, icon: "rps", size: "25px", active: isActive(["/casinos/[network]/[token]/rockpaperscissors", "/casinos/[network]/[token]/rockpaperscissors/[roomid]"]) },
      { label: "Token Burns", href: `/casinos/${network?.name}/${token?.address}/burn`, icon: "burn", size: "25px", active: isActive(["/casinos/[network]/[token]/burn"]) },
      { label: "Bet History", href: `/casinos/${network?.name}/${token?.address}/history`, icon: "clock-rotate-left", size: "25px", active: isActive(["/casinos/[network]/[token]/history"]), className: "text-white/50" },
    ]
    ...
    const _menu2 = [
      ...
      // { label: "Bet History", href: `/history`, icon: "clock-rotate-left", size: "25px", active: isActive(["/history"]) },
    ]
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[200] ">
      <div className="bg-gradient-to-r from-[#161820] to-[#1d1e27] h-[60px] lg:h-[80px] relative px-4 flex items-center gap-2">
        <Link className="header-brand block" href={`/casinos/`}>
          <img src="/logo-letter.png" alt="G" className="block object-contain h-10" />
        </Link>

        <HeaderSearch reportIsFocused={setSearchIsFocused} className="lg:ml-6"/>
        ...
        <div className={`${searchIsFocused ? 'hidden lg:flex' : 'flex'} ml-auto items-center gap-2`}>
          ...
          { user &&
            ...
            >
              <AvatarDarkBg playername={user?.wallet_address} size="35" />
              { <img src={toNetworkImg(network?.id)} alt="" className="absolute top-0 right-[-5px] w-[10px] h-auto"/> }
      ...
      <div className={`${showMenu ? '' : 'hidden lg:flex'} flex flex-col bg-gradient-to-b from-[#1d1e27] to-[#13141b] absolute left-0 top-[60px] lg:top-[80px] w-full lg:w-[80px] h-[calc(100vh-60px)] lg:h-[calc(100vh-80px)]`}>
        <div className={`py-2 lg:w-full lg:flex lg:flex-col lg:items-center ${router.pathname === '/casinos' || router.pathname === '/casinos/[network]' ? 'lg:hidden' : ''}`}>
          {
            menu1.map((x, i) => (              
            <Link onClick={() => setShowMenu(false)} key={i} href={x.href} className="group flex items-center lg:justify-center px-4 py-2 gap-4" data-active={x.active}>
              ...
              <span className={`z-10 ${x.active ? 'text-white' : 'text-white/40 group-hover:text-white'} block lg:hidden lg:absolute font-medium lg:group-hover:block lg:left-[70px] lg:bg-dark-700 lg:border lg:border-gray lg:text-sm lg:py-1 lg:px-2 lg:rounded-lg`}>{x.label}</span>
              { x.active && <span className="hidden lg:block z-[5] absolute right-[-6px] h-[12px] w-[12px] rounded-full bg-primary"></span> }
            </Link>
            ))
          }
        </div>
      ...
      <BalanceAlerts />
      <DepositModal showModal={showDepositModal} setShowModal={setShowDepositModal} />
    </header>
  )
}
