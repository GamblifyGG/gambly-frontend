...
import { CreateGameModal } from '@/components/coinflip'
...
import { getCoinflipGames } from '@/api/coinflip'
...
const CoinFlip = () => {
  ...
  const { token, network, user, setShowDepositModal, setDepositToken, signIn, casinoLoading, casinoError } = useContext(BaseContext)
  ...
  const pageOptions = [
    ...
    { id: 'MyGames', label: 'My Live Games' },
  ...
  const fetchGames = async (chain_id, token_address) => {
    setLoading(true)
    setGames([])
    const [er, data] = await getCoinflipGames({ 
      chain_id, 
      token_address,
      sort: 'created',
      order: sortOrder,
  ...
  useEffect(() => {
    setList(page === 'MyGames' ? games.filter(x => x.creator.wallet_address === user?.wallet_address) : games)
  }, [page, games])
      ...
      <div className="lg:flex items-end gap-3 mb-4">
        <div className="mb-3 lg:mb-0">
          <div className="flex gap-2 items-center text-xl font-semibold">
            ...
            <span className="text-primary">${token?.symbol}</span> Coinflip Games
          </div>
