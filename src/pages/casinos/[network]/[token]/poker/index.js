...
const Poker = () => {
  ...
  const { user, token, setShowDepositModal, setDepositToken, casinoError, casinoLoading } = useContext(BaseContext)
  ...
  const getRooms = async (chain_id, token_address) => {
    ...
    if (er) {
      console.log(er)
      setError(er?.details || er?.error || er?.message)
  ...
  useEffect(() => {
    ...
    if (casinoLoading || casinoError || !token) return

    getRooms(token.network.id, token.address)

    const url =`${process.env.NEXT_PUBLIC_API_URL}casinos/${token.network.id}/${token.address}/poker/live`
    console.log('[SSE]', url);
  ...
  }, [casinoError, casinoLoading, token])
  ...
  return (
    <div className="relative p-4">
      ...
      <div>
        { error && <div className="text-red text-center p-10">
          <p>{error}</p>  
        </div>}
